...
(async function () {
  ...
  const mfiAccount = await tryOrCry(
    mfiClient.createMarginfiAccount(),
  ...
  await tryOrCry(
    mfiAccount.deposit(POSITION_SIZE_USD),
    "Depositing into marginfi account"
  );
...
async function setupZo(mfiAccount: MarginfiAccount) {
  ...
  await tryOrCry(
    zo.deposit(POSITION_SIZE_USD / 2),
...
async function setupMango(mfiAccount: MarginfiAccount) {
  ...
  await tryOrCry(
    mango.deposit(POSITION_SIZE_USD / 2),
    "Depositing into Mango Protocol"
  );
}

async function tryOrCry<G>(
  promise: Promise<G>,
  actionText: string
): Promise<G> {
